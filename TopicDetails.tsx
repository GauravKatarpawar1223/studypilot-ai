...
export default function TopicDetails({ topic, onBack }: Props) {
  const actions = [
    {
      label: 'Learn',
    ...
    <div className="flex min-h-full flex-col px-5 pt-8 pb-6">
      ...
      <div className="mt-3 space-y-3">
        {actions.map(({ label, desc, icon: Icon }) => (
          <button
            key={label}
            onClick={() => alert(`${label} will be powered by the AI agent in a later phase of StudyPilot AI.`)}
      ...
      <p className="mt-auto pt-6 text-center text-[11px] text-ink-400">
        These actions are part of Phase 2. The topic details are ready to connect.
      </p>
    </div>
  );
}
